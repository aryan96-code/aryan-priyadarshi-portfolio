...
export default function BeyondCode() {
  ...
  return (
    <section id="beyond-code" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12">
      ...
      <div className="rounded-2xl bg-gradient-to-r from-emerald-950/40 via-[#0b101c] to-cyan-950/40 border-2 border-teal-500/40 p-8 sm:p-10 shadow-2xl relative overflow-hidden text-center space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        <div className="lg:col-span-4 grid grid-cols-1 gap-4">
          {data.stats.map((stat, idx) => (
            <div key={idx} className="p-6 rounded-2xl bg-[#0b101c] border border-slate-800 space-y-1 text-center sm:text-left hover:border-teal-500/40 transition-colors">
              <span className="text-xs font-mono text-teal-400 uppercase tracking-wider block font-bold">{stat.label}</span>
        ...
        <div className="lg:col-span-8 rounded-2xl bg-[#0b101c] border border-slate-800 p-6 sm:p-8 space-y-6 flex flex-col justify-between shadow-xl">
          <div className="space-y-4">
            <div className="space-y-1">
              <span className="px-3 py-1 rounded bg-teal-500/10 text-teal-300 border border-teal-500/30 text-xs font-mono font-bold">
                {data.dates}
              </span>
